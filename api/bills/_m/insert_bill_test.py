...
async def test_insert_one_return(t: Transaction, log: RequestLogger):
    bill1 = Bill(id=1, name="multi1", value="9999", date=date_sample)
    [res1] = await insert_bill(t, log, bill1)
...
async def test_insert_return(t: Transaction, log: RequestLogger):
    ...
    bill2 = Bill(id=2, name="multi2", value="9999", date=date_sample)
    bill3 = Bill(id=3, name="multi3", value="9999", date=date_sample)
    res1, res2, res3 = await insert_bill(t, log, bill1, bill2, bill3)
    assert res1.name == bill1.name
