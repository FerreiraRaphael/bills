...
awk_result = subprocess.run(awk_command, input=awk_input, stdout=subprocess.PIPE, text=True)
...
class CustomFormatter(logging.Formatter):
    ...
    bold_red = "\x1b[31;1m"
    reset = "\x1b[0m"
    format = "%(levelname)s - %(asctime)s - %(message)s - %(name)s - %(url_path)s - %(file_name)s"
